...
export const contactUnreadCount = catchAsyncErrors(async (_req, res, _next) => {
  const result = await database.query(
    `SELECT COUNT(*)::int AS unread_count FROM contact_messages WHERE is_read = FALSE`
  );
...
export const fetchContactMessages = catchAsyncErrors(async (req, res, _next) => {
...
export const markAllContactMessagesRead = catchAsyncErrors(async (_req, res, _next) => {
  await database.query(`UPDATE contact_messages SET is_read = TRUE WHERE is_read = FALSE`);

  res.status(200).json({
    success: true,
    message: "Contact notifications marked as read.",
  });
});
